...
## Handoff update — 26 August 2026

### Purpose of today's work

Today's work had three connected goals:

1. package and share the opportunity-space market-sizing pipeline;
2. prepare a stakeholder explanation of scoring, market potential, findings,
   uniqueness, features and limitations; and
3. adapt the team's existing Cobalt PowerPoint template without changing its
   design system.

### Git branch status

The market-sizing work was separated from the scoring branch correctly:

```text
Base branch: feature/alec-scoring
New branch:  feature/alec-marketsize
Commit:      f81339e Add opportunity-space market sizing pipeline
Remote:      origin/feature/alec-marketsize
```

The branch is synchronized with the remote. The earlier Git errors had two
causes: `git commit` had no staged changes, and the new branch was pushed before
it existed locally. The corrected workflow was:

```bash
git switch -c feature/alec-marketsize
git add <explicit market-sizing files only>
git diff --cached --stat
git commit -m "Add opportunity-space market sizing pipeline"
git push -u origin feature/alec-marketsize
```

`git diff --cached --stat` opened the `less` pager. Text typed while `(END)` was
visible was interpreted as pager commands. Pressing `q` returned to the shell.

Do not use `git add .` in this repository. The working tree also contains raw
Eurostat downloads, copied teammate repositories, databases, caches, PDFs and
unrelated outputs that must not be added to this feature branch.

### Market-sizing implementation now shared

Commit `f81339e` contains:
...
Analysis/07e_calculate_procurement_benchmark.py
Analysis/07f_build_opportunity_market_size_export.py
Analysis/market_geography.py
Analysis/market_geography_config.json
Analysis/reference/eurostat/
Analysis/reference/market_sizing/
Analysis/outputs/market_sizing/
```

The market object remains:

```text
Opportunity space = Vertical × Use Case × Technology
```

Geography is a calculation and drill-down layer, not a fourth dimension of the
opportunity key. Country cells roll up to Orange Business regions and a
configured Europe headline. France remains its own reporting geography.

The final Beta-app join key is:

```text
vertical|use_case_id|technology_id
```

The current export contains 205 opportunity-space records. All 205 have
`pending_or_unavailable` market-size status because no low/central/high annual
engagement-value assumption has passed the approval gate. This is deliberate
safety behavior, not a pipeline failure.

The old five-country statement immediately above this dated section describes
the earlier technical pilot. The active geography is the expanded configuration
in `market_geography_config.json`; Kilo must use that configuration and the
expanded coverage tables earlier in this document.

### Scoring facts used in the presentation

The presentation uses the actual `05_score_opportunities.py` output:

| Indicator | Verified value |
|---|---:|
| Input evidence records | 576 |
| Eligible evidence records | 338 |
| Excluded records | 238 |
| Opportunity spaces | 205 |
| Radar spaces | 44 |
| Watchlist spaces | 161 |
| Median priority score | 43.2 |

The decision scores must remain separate:

```text
Attractiveness = external signal strength, independence, quality and evidence momentum
Orange fit     = provisional Orange capability-group alignment
Confidence     = evidence volume, quality and source independence
Urgency        = share of regulation, buying-signal and market-move evidence
```

The implemented priority formula is:
...
These are transparent MVP business weights. They are not statistically learned
causal coefficients, win probabilities, revenue forecasts or ROI estimates.

Radar publication currently requires:
...
The highest-priority Radar hypotheses used in the slides are:

| Opportunity space | Exact priority |
|---|---:|
| Public/Gov sector × compliance monitoring × cybersecurity platform | 81.7 |
| Defense × compliance monitoring × cybersecurity platform | 81.1 |
| Healthcare × compliance monitoring × cybersecurity platform | 79.7 |
| Defense × compliance monitoring × IoT platforms | 74.7 |

The presentation rounds these to `82/100`, `81/100`, `80/100` and `75/100`
only because the original slide's score box is too narrow for decimal labels.
The CSV and explanatory Markdown retain exact decimals.

### Business findings and interpretation

The current evidence supports three findings:

1. Compliance monitoring combined with cybersecurity is the strongest repeated
   pattern across regulated verticals, including public sector, defense,
   healthcare and financial services.
2. Operational visibility is a second cluster: asset tracking, environmental
   monitoring, anomaly detection, energy optimization and predictive maintenance
   often combine with IoT or machine learning.
3. Evidence maturity is uneven. Only 44 of 205 spaces pass the Radar gate; the
   161 Watchlist spaces are a research agenda rather than failed opportunities.

Manufacturing has 12 scored opportunity spaces but currently has zero Radar
spaces. Present this as an evidence-coverage gap under the current source mix
and thresholds, not as proof that Manufacturing has no opportunity.

### Attractiveness, Orange fit and right-to-win

Kilo must not rename the current `orange_fit_score` to right-to-win.

Orange fit currently asks whether the evidence matches capability groups Orange
Business could plausibly provide or integrate around. It is calculated from
distinct matched capability groups and is provisional. A full right-to-win
measure still requires:

```text
Orange credentials and certifications
customer references and delivery proof
geographic reach
partner ecosystem
differentiation
competitor strength
...
A concise future formulation is:

```text
Right-to-win = capability fit
             + credentials and references
             + delivery coverage and ecosystem
             + differentiation
             - competitor strength
```

### Market-potential explanation for stakeholders

The scoring model ranks where to investigate. Market potential estimates the
possible annual economic scale. Article count is never used as market size.

For enterprise population `N` and technology-adoption proxy `a`:

```text
Greenfield enterprise base = N × (1 - a)
Expansion / managed-service enterprise base = N × a
```

For each segment:

```text
Annual addressable potential = addressable enterprise base
                             × approved annual engagement value in EUR
```

Low, central and high estimates are published only after validation. A result
must have valid enterprise inputs, an adoption rate between zero and one,
non-negative ordered annual values, EUR currency and an approved assumption.
Rows that fail validation carry null EUR values.

The stakeholder wording should be:
...
Do not call it guaranteed Orange revenue, expected market share, company
valuation or a precise technology-market TAM.

### Presentation files produced today

Two new, currently untracked files are in `Analysis`:

```text
Analysis/Initial_presentation.md
Analysis/Initial_presentation.pptx
```

`Initial_presentation.md` contains the 10-minute structure, a Mermaid pipeline
diagram, speaker scripts, scoring and market-potential formulas, verified
findings, uniqueness, features and limitations, stakeholder questions, and safe
terminology.

`Initial_presentation.pptx` is a content-adjusted copy of:

```text
C:\Users\alec4\Downloads\Innovation_Radar_Cobalt_260826_01.pptx
```

The source PowerPoint in Downloads was not modified.

### PowerPoint formatting incident and correction

The first programmatic edit assigned complete strings through `shape.text`.
That reset run-level formatting in the designer's text boxes, made replaced text
black and unreadable on the dark background, and removed mixed formatting such
as bold white headings followed by normal grey text.

The deck was corrected by rebuilding it from the untouched original and editing
the existing text **runs**, not replacing whole text frames. This preserved:
...
background, borders and visual elements
```

Slide 1 was left unchanged. The corrected deck was opened read-only through
PowerPoint and exported to PNG for visual inspection. Slides were checked for
color, bold/normal hierarchy, wrapping and box boundaries. The only overflow
found was decimal score labels on slide 5; those labels were rounded for display
as documented above. The validated rebuild then replaced
`Analysis/Initial_presentation.pptx`, and temporary files were removed.

Future automated PowerPoint changes must preserve existing runs or explicitly
copy the designer's run formatting. Do not use `shape.text = new_text` on this
template.

### Presentation narrative and ownership

The deck retains the team's original structure:

```text
3 minutes — How it works
2 minutes — Uniqueness
2 minutes — Business findings
3 minutes — Features and limitations
```

Alec's primary sections are Uniqueness and Business findings. The Markdown gives
enough context to explain scoring and market potential during questions.

The central uniqueness statement is:
...
### Immediate next actions for Kilo

1. Do not alter validated formulas or presentation figures without checking the
   current CSV outputs.
2. Treat `Initial_presentation.md` and `Initial_presentation.pptx` as new files;
   they have not yet been committed at the time of this update.
3. When the final Beta-app branch is available, add a market-size service that
   joins by `vertical|use_case_id|technology_id`.
4. Show a Europe low/central/high EUR range only when export status is
   `estimated`; otherwise show the evidence-gap and coverage status.
5. Pass the deterministic market-size object to report generation. AI may
   explain it but must not calculate or invent a missing value.
6. Select a small set of high-priority opportunities and validate their
   greenfield and expansion annual engagement values. Then rerun 07c, 07d and
   07f.
7. Improve source coverage for Manufacturing and validate strategic samples
   rather than attempting to review every article manually.
8. Build right-to-win only after adding approved Orange and competitor evidence;
   do not treat the current Orange-fit score as proof of competitive advantage.

## 2026-08-27: Public-sector pipeline and final Beta-app integration

### Public/Government market-size path in `analysis2`

The Public/Government vertical now has its own denominator instead of being
forced through the enterprise-count method used for commercial verticals.

New preparation file:

```text
analysis2/07a2_prepare_public_sector.py
```

Official public inputs:

```text
nama_10_a64_e : NACE O public-administration employment (context)
gov_10a_exp   : COFOG GF01/GF02 government expenditure (context)
TED records   : distinct opportunity-linked public buyers (denominator)
```

The calculation is:

```text
Observed public buyer base
    = count(distinct normalized buyer, country)
      for recent TED notices assigned to the exact opportunity key

Public annual addressable potential
    = observed public buyer base x approved annual engagement value
```

Employment and expenditure are never substituted for opportunity market size.
They describe the public-sector environment and help an analyst judge whether
the conservative buyer result is plausible. Buyer counts are an observed lower
bound, not a total population of every possible public customer.

Updated pipeline files:

- `market_vertical_config.json` declares `public_buyer_count` for the public
  vertical.
- `market_verticals.py` loads the alternative denominator metadata.
- `07c_prepare_comparable_contract_values.py` adds public low/central/high
  annual-value fields.
- `07d_calculate_market_potential.py` calculates the
  `active_public_procurement` scenario and preserves explicit blockers.
- `07f_build_opportunity_market_size_export.py` exports the public result in the
  same opportunity-key contract used by the UI.

Current result: 205 opportunity records are exported, but all remain pending.
This is intentional: no annual engagement-value row is approved yet. The
pipeline must not display expenditure, employment, enterprise counts or buyer
counts as a euro market size.

### Integration in the isolated final Beta app

The final branch was isolated at:

```text
BeCode_dataOrange-radar-Beta-app-final/
```

The original `Analysis/` folder and the older DemoTest app were not modified.

Runtime import contract:

```text
imports/market_size/opportunity_market_sizes.json
join key = vertical|use_case_id|technology_id
```

The new `radar_v2/services/market_size_repository.py`:

1. loads and indexes the 205 analysis records once;
2. joins them to opportunities without removing unmatched app records;
3. validates that an estimated range is non-negative and ordered
   (`low <= central <= high`);
4. suppresses invalid or pending euro values;
5. converts blockers and coverage into stakeholder-readable labels;
6. clearly labels public employment and expenditure as context, not market
   size.

Coverage against the final Beta import:

```text
Final Beta opportunity keys : 309
Market-size keys            : 205
Exact shared keys           : 205
Beta-only keys              : 104 (kept visible as unavailable)
Market-only keys            : 0
```

The opportunity detail page now contains an opportunity-level annual
addressable-potential card. Attractiveness, Orange Fit and market size remain
separate measures. The report prompt receives the deterministic market-size
record and may quote EUR only when `estimated` is true. It is explicitly told
not to derive a missing market value from contextual or proxy fields.

Tests were added for pending values, valid non-negative ranges, invalid range
suppression and unmatched keys. All four market-size service tests passed with
the standard-library test runner, and Python syntax compilation passed. The
current workspace virtual environment does not contain `pytest`, `PyYAML` or
Reflex, so the complete Beta-app runtime still requires installing the final
app's `requirements.txt` in its own environment.
