...
from market_verticals import load_vertical_config, target_nace_codes
...
TARGET_NACE_CODES = target_nace_codes()
...
def main() -> None:
    ...
    for mapping in VERTICAL_CONFIG["verticals"].values():
        codes = mapping["nace_codes"] or [""]
        for code in codes:
            crosswalk_rows.append({
                "vertical": mapping["vertical"],
                "enabled_for_sbs_estimation": mapping["enabled"],
                "nace_code": code,
                "mapping_quality": mapping["mapping_quality"],
                "statistical_scope": mapping["statistical_scope"],
                "limitation": mapping["limitation"],
                "config_version": VERTICAL_CONFIG["version"],
            })
    crosswalk_path = REFERENCE_DIR / "vertical_nace_crosswalk.csv"
    pd.DataFrame(crosswalk_rows).to_csv(crosswalk_path, index=False, encoding="utf-8-sig")

    print(f"Written: {manifest_path}")
    print(f"Written: {crosswalk_path}")
    print("Use enterprise_counts_standard.csv as the primary denominator.")
    print(f"Configured radar verticals: {len(VERTICAL_CONFIG['verticals'])}")
    print(f"Enabled NACE denominator codes: {len(TARGET_NACE_CODES)}")
    print("Do not add market totals across radar verticals; their NACE scopes can overlap.")
