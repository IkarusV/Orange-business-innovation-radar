...
LOG_PATH = REPO_ROOT / "logs" / "ted_collector.log"
...
def run() -> None:
    setup_logging()
    log = logging.getLogger("ted_collector")
    ...
    for vertical, entry in mapping.items():
        try:
            ...
            verticals_ok += 1
            log.info(
                "OK   [%s] confidence=%s - %d notices, %d new",
                vertical, entry.get("confidence"), len(articles), new_count,
            )
    ...
    conn.close()
    log.info(
        "Run complete: %d verticals OK, %d verticals failed, %d new notices",
